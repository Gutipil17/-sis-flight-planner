...
$('emailBtn').addEventListener('click',async()=>{
  try{
    ...
    const body=[
      ...
      ``,
      `Contacto:`,
      `${settings(s.fullName || s.contactName)}`,
      `Celular: ${settings(s.phone || s.contactPhone)}`,
      `Correo: ${settings(s.email || s.contactEmail)}`,
      ``,
      `Atentamente,`,
      ``,
      `${settings.emailSignature || settings(s.fullName || s.contactName)}`
